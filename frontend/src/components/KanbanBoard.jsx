...
function DraggableTask({ task }) {
  ...
  return <TaskCard task={task} setNodeRef={setNodeRef} style={style} listeners={listeners} attributes={attributes} />;
}
    ...
    <DndContext onDragEnd={handleDragEnd}>
      <div className="grid gap-4 xl:grid-cols-3">
        {columns.map(([id, title, tone]) => (
          <Column key={id} id={id} title={title} tone={tone} tasks={tasks.filter((task) => task.status === id)} />
        ))}
      </div>
    </DndContext>
  );
